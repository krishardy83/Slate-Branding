...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:fw="http://technolutions.com/framework" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="application">
    ...
    <xsl:variable name="url">
      <xsl:choose>
        <xsl:when test="@summary='Event Payment'">https://train.cashnet.com/genw_checkout</xsl:when>
        <xsl:when test="@periodKey='UG' and @summary='Application Fee'">https://train.cashnet.com/euaf_prod</xsl:when>
        <xsl:when test="@periodKey='UG' and @summary='Enrollment Deposit'">https://train.cashnet.com/messiahdepositUslatetest</xsl:when>
        <xsl:when test="@periodKey='GR' and @summary='Application Fee'">
          <xsl:value-of select="concat('https://train.cashnet.com/404Handler/pageredirpost.aspx?virtual=',$virtualVar)" />
        </xsl:when>
    ...
    <form method="post" action="{$url}">
      ...
      <input type="hidden" name="custcode" value="{field[@key='a_banr_id']}" />
      <!-- 
			<input type="hidden" name="custcodetype1" value="custcode" />
      <input type="hidden" name="custcodeval1" value="{@a_banr_id}" />      			
			-->
